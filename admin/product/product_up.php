<?php
  ...
  $product_up_css = '<link rel="stylesheet" href="/attention/admin/product/product_up.css">';
  //$class_cate_css = '<link rel="stylesheet" href="/attention/admin/css/class_cate.css">';
  include_once $_SERVER['DOCUMENT_ROOT'].'/attention/admin/inc/dbcon.php';
  include_once $_SERVER['DOCUMENT_ROOT'].'/attention/admin/product/product_function.php';
...
<div class="common_pd">
  ...
  <form name="frm" action="product_ok.php" method="POST" id="product_form" enctype="multipart/form-data">
          ...
          <td class="lecture_video_area">
            <div class="lecture_video_wrap">
              <div class="lecture_link_insert">
                <input type="text" class="form-control lecture_video_input" placeholder="강의영상 주소를 입력하세요." name="lecture_video[]">
              </div>
              <button type="button" id="lecture_video_add"><i class="bi bi-plus-circle icon_gray"></button>
            </div>
          </td>
        </tr>
...
<?php
    ...
    {
    ?>
                <div id="f_<?php echo $file_row["imgid"][$i]?>"><img src="/attention/pdata/sclass/<?php echo $file_row["filename"][$i]?>" style="width:100px;height:100px;"><span style="color:red" class="del_img"  data-imgid="<?php echo $file_row["imgid"][$i]?>">[삭제]</span></div>
    <?php
    }
?>
            </td>
...
if ($('#cate2').val())
{
  ...
  function attachFile(file){  //이제 attachFile함수가 할 일.
    ...
    $.ajax({
      url: 'product_save_image.php',  // ← 얘한테 일을 시킬거임.
        ...
        if(return_data.result == 'member'){
          ...
          let html = `
            <div class="thumb" id="f_${return_data.imgid}" data-imgid="${return_data.imgid}">
              <img src="/attention/pdata/sclass/${return_data.savefile}" alt="">
  ...
  function file_delete(imgid){
    ...
    $.ajax({
      ...
      type: 'post',
      url: '/attention/admin/product/image_delete.php',
      data: data,
      dataType: 'json',
  ...
  $('#lecture_video_add').click(function(){
    ...
    lecture_video_html = `<div class="lecture_link_insert d-flex align-items-center">${lecture_video_html}</div>`;
