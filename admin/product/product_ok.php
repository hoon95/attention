...
if (in_array($mode, array("add", "edit")))
{
  ...
  //print_r($_FILES);  //첨부파일(썸네일 등)의 경우, 이렇게 확인해야 한다.
    ...
    if($_FILES['thumbnail']['name']){
      ...
      $save_dir = $_SERVER['DOCUMENT_ROOT']."/attention/pdata/sclass/";
      ...
      if(move_uploaded_file($_FILES['thumbnail']['tmp_name'], $save_dir.$thumbnail)) {  //원하는 위치로 잘 옮겨졌다면
        $thumbnail = "/attention/pdata/sclass/".$thumbnail;
      } else {
        echo "<script>
          alert('이미지 등록 실패!');
          history.back();
    ...
    else if ($mode == "edit")
    {
      ...
      if ($mode == "add")
      {
          echo "<script>
          alert('상품 등록 완료!');
          location.href='/attention/admin/product/product_list.php';
          </script>";
      }
      else if ($mode == "edit")
      {
          echo "<script>
          alert('상품 수정 완료!');
          location.href='/attention/admin/product/product_up.php?pid=".$pid."';
          </script>";
      }
    }
  } catch (Exception $e) {
